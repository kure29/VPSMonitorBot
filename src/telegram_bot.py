...
class TelegramBot:
    ...
    async def _handle_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            if data == 'main_menu':
                ...
                if user_id == user_info.id or self._check_admin_permission(user_info.id):
                    await self._show_notification_stats(query, user_id)
                else:
                    await query.answer("❌ 无权限操作", show_alert=True)
            
            elif data.startswith('cooldown_settings_'):
                user_id = data.replace('cooldown_settings_', '')
                if user_id == user_info.id or self._check_admin_permission(user_info.id):
                    await self._show_cooldown_settings(query, user_id)
                else:
                    await query.answer("❌ 无权限操作", show_alert=True)
            
            elif data.startswith('limit_settings_'):
                user_id = data.replace('limit_settings_', '')
                if user_id == user_info.id or self._check_admin_permission(user_info.id):
                    await self._show_limit_settings(query, user_id)
                else:
                    await query.answer("❌ 无权限操作", show_alert=True)
            
            elif data.startswith('quiet_settings_'):
                user_id = data.replace('quiet_settings_', '')
                if user_id == user_info.id or self._check_admin_permission(user_info.id):
                    await self._show_quiet_settings(query, user_id)
                else:
                    await query.answer("❌ 无权限操作", show_alert=True)
    ...
    async def _show_notification_settings(self, message_or_query, user_id: str, edit_message: bool = True) -> None:
        ...
        try:
            ...
            if user:
                # 使用escape_markdown函数来安全处理用户名
                raw_user_display = user.username or user.first_name or f"用户{user_id}"
                user_display = escape_markdown(raw_user_display)
            ...
            if enable_notifications:
                keyboard.extend([
                    ...
                    [
                        InlineKeyboardButton("📊 查看统计", callback_data=f'notification_stats_{user_id}'),
                        InlineKeyboardButton("🔄 重置计数", callback_data=f'reset_daily_count_{user_id}')
                    ]
                ])
            else:
            ...
            keyboard.extend([
                [
                    InlineKeyboardButton("⏰ 冷却设置", callback_data=f'cooldown_settings_{user_id}'),
                    InlineKeyboardButton("📈 限制设置", callback_data=f'limit_settings_{user_id}')
                ],
                [
                    InlineKeyboardButton("🌙 免打扰", callback_data=f'quiet_settings_{user_id}'),
                    InlineKeyboardButton("🧪 测试通知", callback_data=f'test_notification_{user_id}')
                ],
                [InlineKeyboardButton("🏠 返回主菜单", callback_data='main_menu')]
            ])
    ...
    async def _handle_toggle_notifications(self, query, user_id: str) -> None:
        """切换通知开关"""
        try:
            settings = await self.db_manager.get_user_notification_settings(user_id)
            if not settings:
